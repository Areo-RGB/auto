...
export function ensureRefereeJson(): void {
  const jsonPath = REFEREE_JSON_PATH;
...
export function groupRefereeRecords(
...
export function loadRefereeGroups(
  ...
  if (!content) {
    const defaults = cloneDefaultRefereeGroups();
    fs.writeFileSync(jsonPath, JSON.stringify(defaults, null, 2), "utf-8");
    return defaults;
  }
  try {
